...
const MegaMenu = ({ items = [] }) => {
  ...
  const menuWidth = rightColumnWidth + columnCount * contentColumnWidth + contentPadding;
  ...
  return (
    <div
      ...
      className="absolute right-0 top-full z-50 hidden overflow-hidden rounded-b-xl bg-white shadow-[0_12px_32px_rgba(0,0,0,0.16)] group-hover:block"
      style={{
        width: `${finalMenuWidth}px`,
        height: "min(70vh, calc(100vh - 170px))",
    ...
    >
      <div className="flex h-full w-full">
        <aside
          dir="ltr"
          className="mega-menu-scroll h-full w-[200px] shrink-0 overflow-y-scroll overscroll-contain bg-[#f5f5f5]"
        >
          <ul dir="rtl" className="flex min-h-full flex-col items-stretch">
            {items.map((item) => (
              <li key={item.id} className="h-auto w-full p-0">
                ...
                >
                  ...
                  <span className="font-[iran,sans-serif] leading-6">{item.title}</span>
                </button>
              </li>
            ))}
          </ul>
        ...
        <section
          dir="ltr"
          className="mega-menu-scroll h-full flex-1 overflow-y-scroll overscroll-contain bg-white px-6 py-4"
        >
          <div dir="rtl">
            <a
                ...
                <div key={column.id} className="flex min-w-0 flex-col gap-5">
                  {column.groups?.map((group) => (
                    <div key={group.id} className="min-w-0">
                      ...
                      >
                        <span className="shrink-0 text-lg font-light leading-6 text-[#ef4056]">|</span>
                        <span className="min-w-0 text-right leading-6">{group.title}</span>
